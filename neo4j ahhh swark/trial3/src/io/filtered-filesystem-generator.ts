    ...
    try {
      ...
      if (cleanedResponse.batchingRequired && cleanedResponse.batches) {
        ...
        for (const batch of cleanedResponse.batches) {
          ...
          console.log(
            `Generating filtered files for ${batch.description} (${allBatchFiles.length} files)`
          );
          ...
          for (const filePath of allBatchFiles) {
            ...
            try {
              ...
              console.error(
                `Error copying from temp: ${srcFile} -> ${destFile}`,
                err
              );
            }
          }
...
`;
      ...
      console.log(
        "Filtered file system generated successfully:",
        filteredSystemPath
      );
  ...
  static applyContentFiltering(
    ...
    for (const pattern of patterns) {
      try {
        ...
        console.warn(
          `Invalid regex pattern for ${language}: ${pattern}`,
          error
        );
      }
    }

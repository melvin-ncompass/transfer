...
// import StepGenerateGraph from "../Steps/StepGenerateGraph";
...
const Preprocess: React.FC = () => {
  ...
  const handlePreprocess = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/repo/cloneCommit?token=${token}&repo=${selectedRepo}&branch=${selectedBranch}&commit=${selectedCommit}`
      );
      ...
      const metaRes = await fetch(
        `${API_BASE_URL}/preprocess/metadata?repo=${selectedRepo}&username=${username}`
      );
  ...
  const handleBuildPrompt = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/preprocess/buildPrompt?repo=${repo}&username=${username}`
      );
  ...
  const handleAnalyzeWithLLM = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/preprocess/analyzeWithLLM?repo=${repo}&username=${username}`
      );
  ...
  const handleContentFilteration = async () => {
    ...
    try {
      const filterRes = await fetch(
        `${API_BASE_URL}/preprocess/contentFilteration?repo=${repo}&username=${username}`
      );
      const filterData = await filterRes.json();
      if (filterData?.data?.success) {
        const diffRes = await fetch(
          `${API_BASE_URL}/preprocess/repoDiff?repo=${repo}&username=${username}`
        );
  ...
  //       `${API_BASE_URL}/preprocess/generateGraph?repo=${repo}&username=${username}`
  //     );
  ...
  //       `${API_BASE_URL}/preprocess/getBatchSummary?repo=${repo}&username=${username}`
  //     );
  ...
  return (
    <Box sx={{ display: "flex" }}>
      ...
      <Box component="main">
        ...
        <Box sx={{ textAlign: "center", px: 4,width:"80vw",height:"90vh" }}>

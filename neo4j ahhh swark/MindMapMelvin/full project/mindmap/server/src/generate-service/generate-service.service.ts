...
`;
  ...
  async generateWorkflow(body: any) {
    try {
      ...
      const prompt = `
You are an expert software architect and workflow designer. Based on the provided code node data, create a professional workflow diagram data structure.
...
2. Create a logical workflow showing how this function operates
3. Transform it into a sequential process with decision points where appropriate
4. Use the exact JSON format provided below
...
Required Output Format (JSON only, no explanations):
{
  "nodes": [
    ...
    // Add 8-12 more nodes representing the workflow steps
    // Include process nodes, decision nodes, and an end node
    // Use categories: "process", "decision", "start", "end"
  ...
  "links": [
    ...
    // Add links connecting all nodes in logical sequence
    // Use categories: "main", "branch", "feedback"
  ],
  "expandableData": {
    "node-key": [
      {
        ...
        "hasChildren": false,
        "level": 2,
        "description": "Child process description"
      }
    ]
  }
}

Create a workflow that represents the actual execution flow of this function based on its dependencies and purpose. Make it logical and professional.`;

  ...
  const fetchData = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3000/preprocess/listJsonFiles?repo=mindmap&username=${sessionUser()}`
      );
...
const filteredGraphData = selectedGraphData
  ...
  const fetchFunctionDetails = async (relativePath: string) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3000/preprocess/getJsonFile?repo=mindmap&username=${sessionUser()}&relativePath=${encodeURIComponent(
          relativePath
        )}`
      );
      const data = await res.json();
  ...
  const generateFullGraph = async () => {
    ...
    for (const f of files) {
      if (!functionDetails[f.relativePath]) {
        try {
          const res = await fetch(
            `http://localhost:3000/preprocess/getJsonFile?repo=mindmap&username=${sessionUser()}&relativePath=${encodeURIComponent(f.relativePath)}`
          );
          const data = await res.json();

...
export class PreprocessController {
  ...
  @Get('generateJsonFiles')
  async generateJsonFiles(@Query() query: any) {
    try {
      const responseData = (await this.preprocessService.generateJsonFiles(
        query,
      )) as unknown;
      return new ApiResponse(
        responseData,
      ...
      this.logger.error(
        `Error in module preprocess -> generateJsonFiles: ${errorStack(error)}`,
      );
      return new ApiResponse(errorMessage(error), 'error', 500);
    }
  }

  @Get('createNeo4jGraph')
  async createNeo4jGraph(@Query() query: any) {
    try {
      const responseData = (await this.preprocessService.createNeo4jGraph(
        query,
      )) as unknown;
      return new ApiResponse(
        responseData,
      ...
      this.logger.error(
        `Error in module preprocess -> createNeo4jGraph: ${errorStack(error)}`,
      );
      return new ApiResponse(errorMessage(error), 'error', 500);
    }
  }

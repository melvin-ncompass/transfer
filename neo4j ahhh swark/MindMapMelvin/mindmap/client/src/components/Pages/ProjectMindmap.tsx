...
// 🎯 Step 3: Data Fetching from Backend - Updated for shorter connections
    ...
    useEffect(() => {
        const fetchMindmapData = async () => {
            try {
                ...
                // Sort nodes by ID for consistent ordering across refreshes
                ...
                // Find root nodes (nodes with no incoming edges) and sort for consistency
                ...
                const calculatePositions = (node: any, level: number, xOffset: number, visited = new Set()) => {
                    ...
                    const x = level * 400 + 200; // Reduced from 2000 to 400 for shorter connections

                    // More spread out vertically  
                    const y = xOffset * 120 + 200; // Reduced vertical spacing for more compact layout

                    positions.set(node.id, { x, y });

                    // Position children with better vertical distribution
                    let childOffset = xOffset;
                    const childSpacing = Math.max(2, Math.ceil(node.children.length / 3)); // Reduced spacing for more compact layout
                    // Sort children by ID for consistent positioning
                    const sortedChildren = [...node.children].sort((a: any, b: any) => a.id.localeCompare(b.id));
                    sortedChildren.forEach((child: any) => {
                        calculatePositions(child, level + 1, childOffset, visited); // Fixed: pass same visited set
                        childOffset += childSpacing; // Reduced spacing between children
                    });
                };
                ...
                try {
                    ...
                    rootNodes.forEach((root: any) => {
                        ...
                        if (rootNode) {
                            ...
                            rootOffset += Math.max(2, rootNode.children.length + 1); // Reduced spacing for more compact layout
                        }
                    });
                } catch (error) {
                ...
                const transformedNodes: Node[] = nodes.map((node: any, index: number) => {
                    ...
                    if (!pos) {
                        ...
                        pos = {
                            x: col * 350 + 200, // Reduced horizontal spacing for shorter connections
                            y: row * 150 + 200  // Reduced vertical spacing for more compact layout
                        };
                    }
                ...
                const transformedEdges: Edge[] = relationships.map((rel: any, index: number) => ({
                    ...
                    source: rel.source,
                    target: rel.target,
                    type: 'straight', // Changed from 'default' to 'straight' for cleaner lines
                    style: {
                        stroke: '#666', // Changed color to force recompile
                        strokeWidth: 1.5, // Made lines slightly thinner
                        opacity: 0.9
                    },
                ...
                const fallbackEdges: Edge[] = [
                    {
                        ...
                        source: '1',
                        target: '2',
                        type: 'straight', // Keep consistent with main edges

  ...
  private static async copyFilesToTempWithBatches(
    ...
    try {
      ...
      for (const batch of modelOutput.batches!) {
        ...
        console.log(
          `Processing ${batch.description} (${allBatchFiles.length} files)`
        );
...
`;
    ...
    try {
      ...
      for (const [language, files] of Object.entries(
        ...
        for (const filePath of files) {
          // Skip invalid file paths (LLM truncation indicators)
          if (this.isInvalidFilePath(filePath)) {
  ...
  /**
   * Check if a file path is invalid (contains LLM truncation indicators)
   * @param filePath The file path to validate
   * @returns true if the file path is invalid, false otherwise
   */
  private static isInvalidFilePath(filePath: string): boolean {
    // Check for Chinese truncation indicators
    ...
    return chineseIndicators.some(indicator => filePath.includes(indicator));
  }
}

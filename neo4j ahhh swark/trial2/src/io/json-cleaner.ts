  ...
  public static clean(rawInput: string | object): CleanedModelOutput | null {
    try {
      ...
      if (typeof rawInput === "string") {
        ...
        console.log("Raw input contains code block markers:", rawInput.includes('```'));
        ...
        const patterns = [
          ...
          /```\s*(\{[\s\S]*\})\s*```/,      // Code block without json marker
        ...
        // If still starts with ``` try to manually extract
        ...
        console.log("Extracted jsonStr first 100 chars:", jsonStr.substring(0, 100));
        console.log("Extracted jsonStr starts with '{' :", jsonStr.startsWith('{'));
        console.log("Extracted jsonStr ends with '}' :", jsonStr.endsWith('}'));

...
app.post('/api/upload/csv', upload.single('file'), async (req: Request, res: Response) => {
  try {
    ...
    // Detect column types from first few rows of data
    const detectColumnType = (columnName: string): string => {
      ...
      const samples = csvData.slice(0, sampleSize).map(row => row[columnName]);
      
      // Check if all non-empty values are numeric
      const numericValues = samples.filter(val => {
        if (val === null || val === undefined || val === '') return false;
        const num = parseFloat(val);
        return !isNaN(num) && isFinite(num);
      });
      
      // If 80% or more are numeric, treat as numeric
      if (numericValues.length >= sampleSize * 0.8) {
        // Check if all are integers
        const allIntegers = numericValues.every(val => {
          const num = parseFloat(val);
    ...
    // Create table with detected column types
    const columnsDefinition = headers.map((origHeader, idx) => {
      const sanitized = sanitizedHeaders[idx];
      const type = detectColumnType(origHeader);
    ...
    // Get detected types for value conversion
...
app.post('/api/upload/geojson', upload.single('file'), async (req: Request, res: Response) => {
  try {
    ...
    const sanitizedColumns = propertyColumns.map(sanitizeColumnName);

    // Determine geometry type from first feature
    const firstGeometry = geoJsonData.features[0].geometry;
    const geometryType = firstGeometry.type; // Point, LineString, Polygon, etc.

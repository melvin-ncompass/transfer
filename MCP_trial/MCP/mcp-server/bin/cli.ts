...
function loadBootstrap(): (portOverride?: number) => Promise<unknown> {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const mainModule = require('../dist/main.js') as {
    bootstrap: (portOverride?: number) => Promise<unknown>;
  };
...
function getExampleConfigPath(): string {
  const candidates = [
    resolve(__dirname, `../${EXAMPLE_FILE_NAME}`),
    resolve(__dirname, `../../${EXAMPLE_FILE_NAME}`),
    resolve(process.cwd(), 'mcp-server', EXAMPLE_FILE_NAME),
  ];
...
function parseConfig(configPath: string): ToolConfig {
  try {
...
function validateNewDevicePreflight(config: ToolConfig, configPath: string) {
  const issues: string[] = [];
  const hints: string[] = [];
...
async function runInit() {
  ...
  writeFileSync(targetPath, `${JSON.stringify(generatedConfig, null, 2)}\n`, 'utf-8');

...
export class ModelInteractor {
    ...
    private static getConfiguredModel(): string {
        const config = vscode.workspace.getConfiguration("swark.eraser");
        const configuredModel = config.get<string>("languageModel");
        console.log("configuredModel: " + configuredModel);

        if (!configuredModel) {
            throw new Error('Language model is not configured. Please set "swark.eraser.languageModel" setting.');
        }

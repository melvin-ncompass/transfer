...
export class RepositoryReader {
    ...
    private async openFiles(uris: vscode.Uri[]): Promise<File[]> {
        ...
        for (const uri of uris) {
            ...
            // Always include file - let chunking handle token limits
    ...
    private showProcessingMessage(numProcessedFiles: number, totalFiles: number): void {
        if (numProcessedFiles < totalFiles) {
            const skippedFiles = totalFiles - numProcessedFiles;
            vscode.window.showInformationMessage(
                `Analyzing ${numProcessedFiles}/${totalFiles} files (${skippedFiles} files skipped). For better performance on large repositories, try "Swark 2.0: Create Chunked Architecture Diagram"`
            );
        } else {
            vscode.window.showInformationMessage(`Analyzing all ${numProcessedFiles} files`);
        }
    }

...
//    */
//   private static createOutputPaths(
//     outputPath: string,
//     metadata: Swark55ProcessedMetadata
//   ): Swark55OutputPaths {
...
//       semiDetailedEraser: path.join(
//         baseDir,
//         "semi-detailed-architecture.eraser"
//       ),
...
//    */
//   private static async generateSummaryReport(
//     metadata: Swark55ProcessedMetadata,
//     paths: Swark55OutputPaths
//   ): Promise<void> {
...
// **Total File Before Processing:** ${metadata.fileList.length}
// **Worstcase Token Count:** ${metadata.tokenReport.worstCaseTokens.toLocaleString()}
// **Token After file Filtering:** ${metadata.tokenReport.tokensAfterFileFiltering.toLocaleString()}
// **Token After Content Filtering:** ${metadata.tokenReport.tokensAfterContentFiltering.toLocaleString()}

// ## Processing Summary
// - **Total Files Processed:** ${filteredFiles.length}
// - **Total Tokens:** ${filteredFiles
//       .reduce((sum, f) => sum + f.finalTokens, 0)
//       .toLocaleString()}
...
// Primary language **${
//       metadata.detectedLanguages[0]
//     }** detected - diagrams include:
...
//     return (
//       features[language?.toLowerCase() as keyof typeof features] ||
...
//    */
//   private static async generateHighLevelDiagrams(
//     metadata: Swark55ProcessedMetadata,
//     paths: Swark55OutputPaths
//   ): Promise<void> {
//     const criticalFiles = metadata.filteredFiles.filter(
//       (f) => f.importance === "critical-entry-point"
//     );
//     const coreFiles = metadata.filteredFiles.filter(
//       (f) => f.importance === "core-module"
//     );
...
//    */
//   private static async generateSemiDetailedDiagrams(
//     metadata: Swark55ProcessedMetadata,
//     paths: Swark55OutputPaths
//   ): Promise<void> {
//     const importantFiles = metadata.filteredFiles.filter(
//       (f) =>
//         f.importance === "critical-entry-point" ||
//         f.importance === "core-module"
//     );
...
//    */
//   private static async generateDetailedDiagrams(
//     metadata: Swark55ProcessedMetadata,
//     paths: Swark55OutputPaths
//   ): Promise<void> {
//     const d2Request: DiagramRequest = {
...
//       const languageSpecificDiagram = this.generateLanguageSpecificDiagram(
//         metadata,
//         level
//       );
...
//         } catch (llmError) {
//           console.warn(
//             "LLM diagram generation failed, using language-specific fallback:",
//             llmError
//           );
//         }
//       }
...
//    */
//   private static generateLanguageSpecificDiagram(
//     metadata: Swark55ProcessedMetadata,
//     level: string
//   ): string {
//     const primaryLanguage =
//       metadata.detectedLanguages[0]?.toLowerCase() || "unknown";
...
//    */
//   private static generateActualFileArchitecture(
//     metadata: Swark55ProcessedMetadata,
//     level: string
//   ): string {
//     const entryPoints = metadata.filteredFiles.filter(
//       (f) => f.importance === "critical-entry-point"
//     );
//     const coreModules = metadata.filteredFiles.filter(
//       (f) => f.importance === "core-module"
//     );
//     const utilities = metadata.filteredFiles.filter(
//       (f) => f.importance === "supporting-utility"
//     );
...
//         const entryName = this.sanitizeName(
//           path.basename(entry.path, path.extname(entry.path))
//         );
//         coreModules.slice(0, 3).forEach((core) => {
//           const coreName = this.sanitizeName(
//             path.basename(core.path, path.extname(core.path))
//           );
...
//     level: string
//   ): string {
//     return `Generate a ${level} Eraser diagram for this ${
//       metadata.detectedLanguages[0]
//     } project.
...
//     return `title: ${level.toUpperCase()} Architecture - ${
//       metadata.selectedCommit.shortHash
//     }
...
// // Core Modules
// ${coreModules
//   .slice(0, level === "detailed" ? coreModules.length : 5)
//   .map((f) => {
//     const name = path.basename(f.path, path.extname(f.path));
...
//    */
//   private static getImportanceBreakdown(
//     files: Swark55ProcessedMetadata["filteredFiles"]
//   ): string {
...
//    */
//   private static getLanguageBreakdown(
//     files: Swark55ProcessedMetadata["filteredFiles"]
//   ): string {
//     const breakdown = files.reduce((acc, file) => {
...
//       .map(
//         ([language, tokens]) =>
//           `- **${language}**: ${tokens.toLocaleString()} tokens`
//       )
//       .join("\n");

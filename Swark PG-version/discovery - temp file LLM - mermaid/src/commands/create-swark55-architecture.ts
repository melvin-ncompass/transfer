 ...
 * Step 6: Process Batches (build prompt for each batch from temp files, get HIGH-LEVEL business summaries for mindmap) - N LLM calls
...
export async function createSwark55Architecture(): Promise<void> {
  ...
  await vscode.window.withProgress(progressOptions, async (progress, token) => {
    try {
      ...
      progress.report({
        message: "Processing batches for architectural summaries...",
      });
      const summaryFilePath = path.join(outputPath, "batch-summaries.txt");
      await BatchSummaryProcessor.processBatches(
        cleanedResponse,
        tempPath,
        summaryFilePath,
        progress
      ...
      await DiagramGenerator.generateFromSummary(
        summaryFilePath,
        outputPath,
        progress
      );
...
• batch-summaries.txt - Batch analysis summaries from temp files (Step 6 output)

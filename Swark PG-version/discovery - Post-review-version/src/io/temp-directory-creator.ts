  ...
  private static async copyFilesToTempWithBatches(
    ...
    try {
      ...
      for (const batch of modelOutput.batches!) {
        ...
        console.log(`Processing ${batch.description} (${allBatchFiles.length} files)`);
        ...
        for (const filePath of allBatchFiles) {
          ...
          try {
            ...
            const language = this.detectFileLanguage(filePath, modelOutput.detectedLanguages);
        ...
        const batchInfoPath = path.join(batchDir, `batch-${batch.batchNumber}-info.md`);
...
`;
   ...
   */
  private static detectFileLanguage(filePath: string, detectedLanguages: string[]): string {

...
export class JsonCleaner {
  ...
  public static clean(rawInput: string | object): CleanedModelOutput | null {
    try {
      ...
      if (typeof rawInput === "string") {
        ...
        // Remove code block markers if present - more aggressive approach
        let jsonStr = rawInput;
        
        // Try multiple patterns to extract JSON from markdown code blocks
        const patterns = [
          /```json\s*([\s\S]*?)\s*```/,  // Standard markdown code block
          /```\s*([\s\S]*?)\s*```/,      // Generic code block
          /`([\s\S]*?)`/,                // Single backticks
          /^[\s\S]*?(\{[\s\S]*\})[\s\S]*$/ // Extract any JSON object
        ];
  ...
  /**
   * Extract array value from malformed JSON
   */
  private static extractArrayValue(jsonStr: string, key: string): any[] | null {
    try {
      // Special handling for batches array which contains objects
      if (key === 'batches') {
        return this.extractBatchesArray(jsonStr);
      }
      
      // For simple arrays (like detectedLanguages, fileIgnored)
      const pattern = new RegExp(`"${key}"\\s*:\\s*\\[([^\\]]+)\\]`, 'i');
      const match = jsonStr.match(pattern);
      if (match) {
        // Simple array extraction - split by commas and clean quotes
  ...
  /**
   * Extract batches array specifically (handles complex objects)
   */
  private static extractBatchesArray(jsonStr: string): any[] | null {
    try {
      // Try to find the batches array with a more sophisticated approach
      const batchesPattern = /"batches"\s*:\s*\[([\s\S]*?)\]/;
      const match = jsonStr.match(batchesPattern);
      
      if (match) {
        const batchesContent = match[1];
        
        // Try to extract individual batch objects
        const batches = [];
        const batchPattern = /\{[^{}]*(?:\{[^{}]*\}[^{}]*)*\}/g;
        let batchMatch;
        
        while ((batchMatch = batchPattern.exec(batchesContent)) !== null) {
          try {
            // Try to parse each batch object
            const batchStr = batchMatch[0];
            const batchObj = JSON.parse(batchStr);
            batches.push(batchObj);
          } catch (e) {
            // If parsing fails, create a fallback batch object
            console.warn('Failed to parse batch object, creating fallback');
            batches.push({
              batchNumber: batches.length + 1,
              files: [],
              estimatedTokens: 0,
              description: 'Batch with parsing error'
            });
          }
        }
        
        // If no valid batches found, create some fallback batches
        if (batches.length === 0) {
          console.warn('No valid batches extracted, creating fallback batches');
          for (let i = 1; i <= 5; i++) {
            batches.push({
              batchNumber: i,
              files: [],
              estimatedTokens: 1000,
              description: `Fallback batch ${i}`
            });
          }
        }
      ...
      console.warn('Error extracting batches array:', error);
      return null;
    }
  }

  /**
   * Extract object value from malformed JSON (simplified)
   */

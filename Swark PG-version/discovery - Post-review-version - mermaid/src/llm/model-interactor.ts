...
export class ModelInteractor {
  ...
  public static async getModel(): Promise<vscode.LanguageModelChat> {
    ...
    this.sendModelSelectedTelemetry(
      selectedModel,
      configuredModel,
      fallbackModel,
      availableModels
  ...
  private static async getAvailableModels(): Promise<
  ...
  private static getConfiguredModel(): string {
    ...
    if (!configuredModel) {
      throw new Error(
        'Language model is not configured. Please set "swark.languageModel" setting.'
      );
    }
  ...
  private static selectModel(
    ...
    const configuredModel = this.findModel(
      availableModels,
    ...
    if (fallbackModel) {
      this.showModelNotAvailableMessage(
        configuredModelFamily,
        fallbackModelFamily,
        availableModels
      );
  ...
  private static availableModelsToString(
    availableModels: vscode.LanguageModelChat[]
  ): string {

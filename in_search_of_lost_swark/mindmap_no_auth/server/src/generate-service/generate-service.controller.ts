import { Controller, Get, Logger, Param, Post, Query, Body } from '@nestjs/common';
...
export class GenerateServiceController {
  ...
  @Post('generateWorkflow')
  async generateWorkflow(@Body() body: any) {
    try {
      const responseData = await this.generateService.generateWorkflow(body);
      return new ApiResponse(responseData, 'Workflow generated', 200);
    } catch (error) {
      this.logger.error(
        `Error in module flowchart -> generateWorkflow: ${error.stack}`,
      );
      return new ApiResponse(error.message, 'error', 500);
    }
  }

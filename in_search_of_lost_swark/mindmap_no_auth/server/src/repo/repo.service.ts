...
export class RepoService {
  ...
  async getUserProfile(query: { token: string }) {
    // Mock user profile for No-Auth mode
    return {
      login: 'local',
      id: 12345,
      avatar_url: '',
      gravatar_id: '',
      url: 'https://api.github.com/users/local',
      html_url: 'https://github.com/local',
      name: 'Local User',
      email: 'local@mindmap.com',
    };
  }
  ...
  async cloneLocalRepo(
    ...
    //   );
    // }
    const gitDir = extractedDir; // Treat extracted root as git root
  ...
  async cloneLocalPath(body: { userName: string; repoName: string; repoPath: string }) {
    const { userName, repoName, repoPath } = body;
    const USERS_ROOT: any =
      this.configService.get<string>('USERS_ROOT') || process.env.USERS_ROOT;
    
    // Validate source path
    if (!fs.existsSync(repoPath)) {
        throw new BadRequestException(`Source path does not exist: ${repoPath}`);
    }

    const baseDir = path.join(USERS_ROOT, userName, repoName);
    const extractedDir = path.join(baseDir, 'extracted');

    fs.mkdirSync(baseDir, { recursive: true });
    
    // Clear existing directory if it exists to ensure fresh copy
    ...
    try {
        console.log(`Copying files from ${repoPath} to ${extractedDir}`);
        fsExtra.copySync(repoPath, extractedDir, {
            filter: (src) => {
    ...
    // Treat the extracted directory as the git root (or source root)
    const gitDir = extractedDir;

    // Create/Update User (Mock profile or reuse logic)
     let user = await this.userRepository.findOne({
      where: { username: userName },
    });
    ...
    let project = await this.projectRepository.findOne({
        where: { projectname: repoName, user: { id: user.id } },
        relations: ['user'],
    });
    ...
    await this.projectRepository.save(project);
     ...
     return {
      extractedTo: extractedDir,
      gitFoundAt: gitDir,
      status: 'Local files copied successfully',
      userId: user.id,
      projectId: project.id,
    };
  }

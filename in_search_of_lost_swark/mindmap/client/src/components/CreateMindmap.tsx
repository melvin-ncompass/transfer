...
const CreateMindmap: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/repo/allRepos?token=${token}`, {
  ...
  const fetchBranches = (repoName: string) => {
    ...
    setLoading(true);
    fetch(
      `${API_BASE_URL}/repo/repoBranches?token=${token}&repo=${repoName}`,
      { headers: { Authorization: `Bearer ${token}` } }
    )
      .then((res) => res.json())
      .then((response) => {
  ...
  const fetchCommits = (repoName: string, branch: string) => {
    ...
    fetch(
      `${API_BASE_URL}/repo/branchCommits?token=${token}&repo=${repoName}&branch=${branch}`,
      { headers: { Authorization: `Bearer ${token}` } }
    )
      .then((res) => res.json())
      .then((response) => {
  ...
  try {
    const res = await fetch(
      `${API_BASE_URL}/repo/cloneLocalRepo?repo=${localRepo}userName=${username}`,
      {
        method: "POST",
        body: formData,
      }

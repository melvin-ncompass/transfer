...
const DynamicWorkflowVisualization: React.FC = () => {
  ...
  const generateWorkflow = async (nodeData: any) => {
    try {
      ...
      console.log('👥 Node has children:', nodeData.hasChildren);
      console.log('👥 Children count:', nodeData.children?.length || 0);
      if (nodeData.children?.length > 0) {
        console.log('👥 Children details:', nodeData.children.map((child: any) => ({
          id: child.id,
      ...
      if (result.statusCode === 200 && result.data?.workflowData) {
        ...
        const workflowValidation = result.data.workflowValidation;
        if (workflowValidation) {
          console.log(`📊 Workflow Validation:`);
          console.log(`   - Functions Analyzed: ${workflowValidation.totalFunctionsAnalyzed}`);
          console.log(`   - Required Minimum Nodes: ${workflowValidation.minimumRequiredNodes}`);
          console.log(`   - Actual Nodes Generated: ${workflowValidation.actualNodesGenerated}`);
          console.log(`   - Meets Requirement: ${workflowValidation.meetsRequirement ? '✅ YES' : '❌ NO'}`);
          if (workflowValidation.functionNodeMapping?.length > 0) {
            console.log(`   - Function Mapping:`, workflowValidation.functionNodeMapping);
          }
        }
        ...
        // Log children file analysis results
        const childrenAnalysis = result.data.childrenFileAnalysis;
        if (childrenAnalysis) {
          console.log(`� Function Dependencies: ${childrenAnalysis.functionsFound}/${childrenAnalysis.functionsExtracted} functions extracted`);
          console.log(`� Max Depth Reached: ${childrenAnalysis.maxDepthReached}`);
          console.log(`🔧 Function Details:`, childrenAnalysis.functionDetails);
          if (childrenAnalysis.errors > 0) {
            console.warn(`� Function Extraction Errors:`, childrenAnalysis.errorDetails);
          }
        }
        ...
        // Log workflow validation results for fallback case
        const workflowValidation = result.data.workflowValidation;
        if (workflowValidation) {
          console.log(`📊 Workflow Validation (fallback):`);
          console.log(`   - Functions Analyzed: ${workflowValidation.totalFunctionsAnalyzed}`);
          console.log(`   - Required Minimum Nodes: ${workflowValidation.minimumRequiredNodes}`);
          console.log(`   - Actual Nodes Generated: ${workflowValidation.actualNodesGenerated}`);
          console.log(`   - Meets Requirement: ${workflowValidation.meetsRequirement ? '✅ YES' : '❌ NO'}`);
          if (workflowValidation.functionNodeMapping?.length > 0) {
            console.log(`   - Function Mapping (fallback):`, workflowValidation.functionNodeMapping);
          }
        }
        ...
        // Log children file analysis results for fallback case too
        const childrenAnalysis = result.data.childrenFileAnalysis;
        if (childrenAnalysis) {
          console.log(`� Function Dependencies (fallback): ${childrenAnalysis.functionsFound}/${childrenAnalysis.functionsExtracted} functions extracted`);
          console.log(`� Max Depth Reached (fallback): ${childrenAnalysis.maxDepthReached}`);
          console.log(`🔧 Function Details (fallback):`, childrenAnalysis.functionDetails);
          if (childrenAnalysis.errors > 0) {
            console.warn(`� Function Extraction Errors (fallback):`, childrenAnalysis.errorDetails);
          }
        }

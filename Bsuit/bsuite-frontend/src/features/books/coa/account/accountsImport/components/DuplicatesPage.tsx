...
import { useUpdateDuplicatesAccountsMutation } from "../api/accountsImport.api";
import { resetAccImpState, setDuplicatesToCreate, setServerDuplicates } from "../CSVSlice";
...
export default function DuplicatesPage() {
  ...
  const serverDuplicates = useAppSelector((s) => s.accImport.serverDuplicates) || [];
  const mappedData = useAppSelector((s) => s.accImport.data) || [];
  const [updateDuplicates, { isLoading }] = useUpdateDuplicatesAccountsMutation();
  ...
  function findRowIndexByData(itemData: any) {
    const candidates = ["accountName", "accountCode"];
    for (let i = 0; i < mappedData.length; i++) {
      ...
      for (const key of candidates) {
        ...
        const itemVal = String(itemData?.[key] ?? "").toLowerCase();

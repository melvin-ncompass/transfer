  ...
  return [
    {
      ...
      label: "Bill Date",
      flex: 1,
      render: (row) => formatDateShort(row.billDate),
      width: "10%",
    },
    {
      id: "billNo",
    ...
    },
    {
      ...
      label: "Status",
      flex: 1,
      render: (row) => <BillStatusBadge row={row} />,
      width: "10%",
    },
    {
      id: "dueIn",
      label: "Due In",
      flex: 1,
      render: (row) => <BillDueInCell row={row} />,
      width: "10%",
    },
    {
      id: "billTotal",
      ...
      flex: 1,
      render: (row) => {
        const currencySymbol = row.billCurrency
          ? row.billCurrency.split(" - ")[0]
          : "₹";

        return formatAmount(Number(row.billTotal || 0), currencySymbol);
      },
      width: "10%",
    },
    {
...
/* ---------- Status Badge ---------- */
const BillStatusBadge = ({ row }: { row: any }) => {
  const dueDate = new Date(row.billDueDate);
  ...
  if (row.balanceDue <= 0) {
    badgeLabel = "Paid";
  } else if (row.balanceDue === Number(row.billTotal) && dueIn >= 0) {
    badgeLabel = "Unpaid";
  } else if (row.balanceDue > 0 && dueIn < 0) {
    badgeLabel = "Overdue";
  }

  // Map to actual theme colors
...
/* ---------- Due In Cell ---------- */
const BillDueInCell = ({ row }: { row: any }) => {
  const dueDate = new Date(row.billDueDate);
  const today = new Date();
...
/* ---------- Hoverable Bill No (Items Popper) ---------- */
const BillNoHover = ({ row }: { row: any }) => {

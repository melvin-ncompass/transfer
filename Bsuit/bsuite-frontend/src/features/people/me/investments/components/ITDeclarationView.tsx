...
import { POICommentsDrawer } from "./POICommentsDrawer";
...
} from "../utils/itDeclarationTableRows";
import { renderPoiCountIconButton } from "../utils/poiCountIconButton";
import { usePoiScopedComments } from "../hooks/usePoiScopedComments";
import {
...
/** Fetches a single file blob and renders it as an image or PDF placeholder */
function ProofCarouselItem({ proof }: { proof: POIAttachment }) {
  const {
    data: blob,
    isLoading,
    isError,
  } = useGetAttachmentFileQuery(proof.path);
...
/** Carousel over an array of POIAttachments using getAttachmentFile per slide */
...
type ITDeclarationViewProps = {
  /** When provided, refetch IT declaration APIs each time this Me tab becomes active. */
...
export default function ITDeclarationView({
  ...
  const { data: employeeInfo, isLoading: isLoadingEmployee } =
    useGetEmployeeInfoQuery();

  const [openLetOutRows, setOpenLetOutRows] = useState<{ [id: string]: boolean }>({});
  const employeeId = Number(employeeInfo?.data?.employeeId);
  ...
  /**
   * Refetch whenever the Investment tab is active. Also re-runs when employeeId /
   * activeFY become available (avoids missing declared data if FY loads after tab open).
   */
  useEffect(() => {
  ...
  const handleSaveComment = async () => {
    ...
    try {
      ...
      setSnackbar({ message: error?.data?.message ?? error?.error ?? error?.message ?? "Failed to add comment.", color: "error" });
    }
  };

  const handleEditDeclaration = () => {
    navigate(`/people/investment/edit?tab=3`, {
  ...
  const handleUploadProofs = async () => {
    ...
    if (!declaredData?.id) return;
    if (!drawerContent?.entityType || !drawerContent.originalId) {
      setSnackbar({
    ...
    try {
      ...
      setSnackbar({ message: err?.data?.message ?? err?.error ?? err?.message ?? "Upload failed.", color: "error" });
    }
  };
  ...
  const header = (
    <>
      ...
      >
        ...
        >
          {isLoadingEmployee || isLoadingFY || (isLoading && !declaredData) ? (
            ...
            <Box width={200}>
              <SingleSelectElement
                ...
                value={activeFY}
                onChange={(value) => setFySelectValue(value ?? "")}
                options={fyOptions}
              />
            </Box>
          )}
  ...
  return (
    <>
      <Box p={3}>
          ...
          <Box textAlign="center">
            ...
            {!isDeclarationReadOnly && (
              <Button
                variant="contained"
                onClick={() =>
                  navigate(`/people/investment/add?tab=3`, {
        ...
        {/* ── Proof of Investment Modal (AttachmentsModal-style) ── */}
        <ModalElement
          open={proofModalOpen}
        ...
        >
          ...
          >
            ...
            {/* ── Right panel: Existing proofs carousel ── */}
            {attachmentsLoading ? (
              <Box
                sx={{
                  flex: 1,

...
import type {
  RawTaxExemptionSection,
  RawTaxExemptionSubsection,
  CreateITDeclarationDto,
  RentedHouseDetailDto,
  SelfOccupiedPropertyDto,
  LetOutPropertyDto,
  ExemptionDetailDto,
  DeclaredDataForFY,
...
function pick80CPrincipalSubsection(
  ...
  const nameMatch = subs.find((s) => {
    const n = (s.exemptionName ?? "").toLowerCase();
...
export default function ITDeclarationModal({ setcurrentTab }: { setcurrentTab?: (tab: number) => void }) {
  ...
  // Block the whole page until we know whether HRA section should render — avoids flicker.
  if (isLoadingDeclaration) {
    return (
      ...
      >
        <CustomCircularProgress size={48} />
      </Card>
    );
  }
    ...
    >
      ...
      >
        <Stack direction="row" alignItems="center" spacing={2}>
          <IconButton
            onClick={() => setcurrentTab ? setcurrentTab(3) : navigate(-1)}
            size="small"
            sx={{
              bgcolor: "white",
              border: "1px solid",
                            ...
                            <Grid container spacing={2}>
                              <Grid size={{ xs: 12, sm: 6 }}>
                                <TextFieldElement
                                  ...
                                  onChange={(e) => {
                                    const raw = parseNumberForTyping(e.target.value);
                                    handleChange("principalPaidInput", raw);
                                    if (raw === "") handleChange("principalPaid", "");
                                  }}
                                  onBlur={() => {
                                    const raw =
                                      item.principalPaidInput !== ""
                              ...
                              <Grid size={{ xs: 12, sm: 6 }}>
                                <TextFieldElement
                                  ...
                                  onChange={(e) => {
                                    const raw = parseNumberForTyping(e.target.value);
                                    handleChange("interestOnLoanInput", raw);
                                    if (raw === "") handleChange("interestOnLoan", "");
                                  }}
                                  onBlur={() => {
                                    const raw =
                                      item.interestOnLoanInput !== ""

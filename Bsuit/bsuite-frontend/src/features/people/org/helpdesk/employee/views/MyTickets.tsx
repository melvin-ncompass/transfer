...
import { Add,  Filter, FilterList } from "@mui/icons-material";
...
import CreateTicketModal from "../../employer/components/CreateTicketModal";
import { useNavigate } from "react-router-dom";
import { useGetMyTicketsQuery } from "../../api/ticket.api";
...
const ticketColumns: GridColDef[] = [
    {field:"select",renderCell:()=><Checkbox />,renderHeader:()=><Checkbox  />},
...
function MyTickets() {
  ...
  // Filter tickets by status slug or status name
  const openTickets = mappedTickets.filter((t) => {
    const slug = t._slug.toLowerCase();
    const status = t.ticketStatus.toLowerCase();
    return slug !== "closed" && slug !== "resolved" && status !== "closed" && status !== "resolved";
  });

  const closedTickets = mappedTickets.filter((t) => {
    const slug = t._slug.toLowerCase();
    const status = t.ticketStatus.toLowerCase();
    return slug === "closed" || slug === "resolved" || status === "closed" || status === "resolved";
  });
  ...
  return (
    <Box height="90%" overflow="scroll" p={2}>
      ...
      <SectionHeader
        title="Open Tickets"
        subtitle="These are your tickets that are yet to be addressed."
        onAdd={() => {
          console.log("Add a new ticket");
      ...
      <SectionHeader
        ...
        subtitle="These are your tickets that have been addressed."
        mt={4}
      />
      <QuickFilterDataGrid
        columns={ticketColumns}
...
function SectionHeader({ title, subtitle, onAdd, mt = 2 }: SectionHeaderProps) {

...
export class Swark55RegexGenerator {
     ...
     */
    public static async generatePatterns(metadata: Swark55Metadata): Promise<Swark55RegexPatterns> {
        try {
...
${metadata.fileList.slice(0, 20).map(f => `${f.path} (${f.language}, ${f.importance})`).join('\n')}
...
Analyze the repository and provide optimized patterns for this specific codebase.`;
     ...
     */
    private static parseRegexResponse(response: string, detectedLanguages: string[]): Swark55RegexPatterns {
        try {
     ...
     */
    private static validateAndCleanPatterns(parsed: any, detectedLanguages: string[]): Swark55RegexPatterns {
        const result: Swark55RegexPatterns = {
            fileInclusion: Array.isArray(parsed.fileInclusion) ? parsed.fileInclusion : [],
            fileExclusion: Array.isArray(parsed.fileExclusion) ? parsed.fileExclusion : [],
            contentFilters: {}
        };
        ...
        for (const language of detectedLanguages) {
            if (parsed.contentFilters && parsed.contentFilters[language]) {
                result.contentFilters[language] = {
                    loggingStatements: Array.isArray(parsed.contentFilters[language].loggingStatements) 
                    ...
                    unusedImports: Array.isArray(parsed.contentFilters[language].unusedImports) 
                        ? parsed.contentFilters[language].unusedImports : [],
                    boilerplate: Array.isArray(parsed.contentFilters[language].boilerplate) 
                        ? parsed.contentFilters[language].boilerplate : []
                };
            } else {
                result.contentFilters[language] = this.getLanguageDefaultPatterns(language);
            }
        }
     ...
     */
    private static extractPatternsFromText(response: string, detectedLanguages: string[]): Swark55RegexPatterns {
        ...
        const exclusionPatterns = response.match(/(?:test|dist|build|node_modules)\/\*\*/g) || [];
     ...
     */
    private static getFallbackPatterns(detectedLanguages: string[]): Swark55RegexPatterns {
        const result: Swark55RegexPatterns = {
            fileInclusion: [
                'src/**/*',
                'lib/**/*',

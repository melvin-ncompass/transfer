...
import { LLMFileClassifier, FileClassificationRequest } from '../llm/file-classifier';
...
export class Swark55MetadataExtractor {
     ...
     */
    public static async extractMetadata(repositoryPath: string, selectedCommit: GitCommit): Promise<Swark55Metadata> {
        // Scan directory structure
        const directoryStructure = await this.buildDirectoryStructure(repositoryPath);
        
        // Get all files with basic info
        const allFiles = await this.getAllFiles(repositoryPath);
        
        // Detect languages dynamically using LLM
        const detectedLanguages = await LLMLanguageDetector.detectLanguages(allFiles);
        ...
        for (const filePath of allFiles) {
            const fileInfo = await this.analyzeFile(repositoryPath, filePath, detectedLanguages);
            if (fileInfo) {
        ...
        const dependencyGraph = await DependencyAnalyzer.buildDependencyGraph(repositoryPath, fileList);
        
        // Use LLM to classify file importance dynamically
        await this.classifyFileImportanceWithLLM(fileList, dependencyGraph, detectedLanguages, directoryStructure);
    ...
    private static ensureImportantFiles(
        ...
        const entryPoints = fileList.filter(f => f.importance === 'critical-entry-point');
     ...
     */
    private static findMainFileForLanguage(fileList: Swark55FileInfo[], language: string): Swark55FileInfo | null {
        ...
        for (const pattern of genericMainPatterns) {
            const found = fileList.find(f => 
                pattern.test(path.basename(f.relativePath)) && 
                f.language === language
            );
            if (found) return found;
        }
     ...
     */
    private static async buildDirectoryStructure(repositoryPath: string, maxDepth: number = 3): Promise<string> {
        const structure: string[] = [];
        
        const buildTree = async (currentPath: string, prefix: string = '', depth: number = 0): Promise<void> => {
        ...
        const scanDirectory = async (currentPath: string): Promise<void> => {
            try {
                ...
                for (const item of items) {
                    ...
                    if (excludedDirectories.has(item) || (item.startsWith('.') && item !== '.gitignore')) {
                        continue;
                    }
     ...
     */
    private static async analyzeFile(repositoryPath: string, filePath: string, detectedLanguages: string[]): Promise<Swark55FileInfo | null> {
        try {
            const stats = await stat(filePath);
            const relativePath = path.relative(repositoryPath, filePath);
            const language = LLMLanguageDetector.detectFileLanguage(relativePath, detectedLanguages);
            ...
            const estimatedTokens = Math.ceil(stats.size / 4); // Rough token estimation
    ...
    private static async classifyFileImportanceWithLLM(
        ...
        try {
            ...
            const response = await LLMFileClassifier.classifyFiles(classificationRequest);
            ...
            console.log(`LLM classified ${response.classifications.length} files across ${detectedLanguages.join(', ')} languages`);
            
        } catch (error) {
            console.warn('LLM file classification failed, falling back to heuristic classification:', error);
            
            // Fallback to basic heuristic classification
            await this.fallbackClassifyFileImportance(fileList, dependencyGraph, detectedLanguages);
        }
    }
    ...
    private static async fallbackClassifyFileImportance(
        ...
        for (const file of fileList) {
            ...
            const isEntryPoint = entryPointPatterns.some(pattern => pattern.test(fileName));
            if (isEntryPoint) {
            ...
            const isCoreModule = corePatterns.some(pattern => pattern.test(file.relativePath));
            if (isCoreModule) {
                file.importance = 'core-module';
                continue;
            }
            ...
            const isUtility = utilityPatterns.some(pattern => pattern.test(file.relativePath));
            if (isUtility) {

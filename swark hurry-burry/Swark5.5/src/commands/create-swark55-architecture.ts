...
import { Swark55ContentFilter, Swark55ProcessedMetadata } from '../io/swark55-content-filter';
    ...
    await vscode.window.withProgress(progressOptions, async (progress, token) => {
        try {
            ...
            if (!workspaceFolders || workspaceFolders.length === 0) {
                vscode.window.showErrorMessage('❌ No workspace folder found. Please open a repository.');
                return;
            }
            ...
            if (!await GitUtils.isGitRepository(repositoryPath)) {
                vscode.window.showErrorMessage('❌ Selected folder is not a git repository.');
                return;
            }
            ...
            if (!userSelectedFolder || userSelectedFolder.length === 0) {
                vscode.window.showErrorMessage('❌ No output folder selected. Analysis cancelled.');
                return;
            }
            ...
            if (!selectedCommit) {
                vscode.window.showInformationMessage('❌ No commit selected. Analysis cancelled.');
                return;
            }
            ...
            progress.report({ message: "Generating intelligent filtering patterns..." });
            ...
            await Swark55OutputGenerator.generateOutputs(processedMetadata, outputPath);
...
async function selectCommit(commits: GitCommit[]): Promise<GitCommit | undefined> {
...
async function showResults(metadata: Swark55ProcessedMetadata, outputPath: string): Promise<void> {
    const totalTokens = metadata.filteredFiles.reduce((sum, f) => sum + f.finalTokens, 0);

...
export class RoleService {
  ...
  async findAllRoles(status: ArchiveStatus, query: PaginationDto) {
    ...
    try {
      ...
      const enrichedRoles = roles.map((role) => {
        ...
        return {
          ...
          users:role.userMappings?.map((m) => m.user).filter((u) => status === ArchiveStatus.GET_ALL || !u.isArchived) ?? []
  ...
  async softDeleteRole(roleId: string) {
    ...
    try {
      ...
      if (role.name === DefaultRoleEnum.USER || role.name === DefaultRoleEnum.SUPER_ADMIN)
        throw new ForbiddenException(
          `Cannot delete the default ${role.name} role`,
        );

      const fallbackRole = await roleRepository.findOne({
        where: { name: DefaultRoleEnum.USER },
        relations: ['permissionMappings', 'permissionMappings.permission'],
      });

      if (!fallbackRole)
        throw new NotFoundException(`Fallback ${DefaultRoleEnum.USER} role not found`);
  ...
  async getPermissionsForRole(roleName: string): Promise<SysPermission[]> {
    const role = await this.roleRepo.findOne({
      where: { name: roleName },
      relations: ['permissionMappings', 'permissionMappings.permission','visualizationPermissionMappings','visualizationPermissionMappings.permission'],
    });
    ...
    return [...role.permissionMappings, ...role.visualizationPermissionMappings].map(
      (rp) => rp.permission.name as unknown as SysPermission,
    );
  }

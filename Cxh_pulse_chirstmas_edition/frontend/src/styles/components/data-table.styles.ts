...
export const dataTableStyles = {
   ...
   */
  tableContainer: (tableMaxHeight: string, shouldEnableScrolling: boolean, notFound: boolean, isEmpty: boolean): SxProps<Theme> => ({
  ...
  tableCell: (isChangeColumn: boolean, isCountColumn: boolean, col: any, onRowClick: any, renderExpandedRow: any): SxProps<Theme> => ({
    ...
    maxWidth: col.maxWidth ?? (isChangeColumn || isCountColumn ? '120px' : '200px'),
    minWidth: col.minWidth ?? (isChangeColumn || isCountColumn ? '100px' : 'auto'),
    width: col.width ?? 'auto',
    display: 'table-cell',

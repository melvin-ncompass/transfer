...
export const BrandingProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [occasion, setOccasionState] = React.useState(() => localStorage.getItem('branding_occasion') || 'default');

  const setOccasion = (newOccasion: string) => {
    setOccasionState(newOccasion);
    localStorage.setItem('branding_occasion', newOccasion);
  };
  ...
  const contextValue = useMemo(() => ({
    ...
    refresh: () => refetch(),
    setOccasion,
  }), [branding, isLoading, fetchError, refetch, setOccasion]);

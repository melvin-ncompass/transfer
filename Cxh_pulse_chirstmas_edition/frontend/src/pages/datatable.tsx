...
import { getDateRangeMinDate, getDateRangeMaxDate, getDateLastYearDate } from '../store/constants';
...
export default function DataTablePage() {
  ...
  // Fetch data from API
  const { data: apiData, isLoading, isSuccess } = useGetDataTableQuery(apiParams);
  ...
  const columns: DataTableColumn[] = [
    ...
    { id: 'precip', label: 'Precipitation (mm)', align: 'right' },
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', 'ward-level-data.csv');
    ...
    <Box sx={datatablePageStyles.container}>
      <Stack spacing={1}>
        ...
        <DataTable
          ...
          rightAction={<PrimaryButton onClick={handleExportCSV}>Export CSV</PrimaryButton>}

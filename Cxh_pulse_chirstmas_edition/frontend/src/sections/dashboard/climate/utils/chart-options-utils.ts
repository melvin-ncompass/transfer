...
export function buildChartOptions({
    ...
    setClickedDate,
    buildTooltipContent,
}: ChartOptionsParams): ChartOptions {
    return {
        chart: {
            zoom: {
                enabled: false,
            ...
            toolbar: {
                show: false, // Hide toolbar completely
            },
            events: {
        ...
        markers: {
            ...
            discrete: clickedPointIndex >= 0 ? [
                ...(showTemperature && temperatureChartData[clickedPointIndex]?.y != null && !isNaN(temperatureChartData[clickedPointIndex]?.y) ? [{
                    seriesIndex: 0,
                ...
                ...(showPrecipitation && precipitationChartData[clickedPointIndex]?.y != null && !isNaN(precipitationChartData[clickedPointIndex]?.y) ? [{
                    seriesIndex: 1,
                    dataPointIndex: clickedPointIndex,
                    fillColor: '#fff',
        ...
        annotations: {
            xaxis: clickedPointIndex >= 0 ? [{
                x: temperatureChartData[clickedPointIndex]?.x || precipitationChartData[clickedPointIndex]?.x,
                strokeDashArray: 4,
                borderColor: '#999',
                borderWidth: 1,
                opacity: 0.6,

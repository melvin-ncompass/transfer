...
export default function SettingsPage() {
  ...
  const { refresh: refreshBranding, setOccasion: setContextOccasion, branding: contextBranding } = useBranding();
  ...
  const hasBrandingChanges =
    ...
    occasion !== originalBranding.occasion;
  ...
  const handleDeactivate = () => {
    ...
    setSnackbar({ open: true, message: 'Account deactivation request received', severity: 'success' });
  ...
  const handleSaveEmail = async () => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: 'Email configuration saved', severity: 'success' });
  ...
  const handleSaveStorage = async () => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: 'Storage path saved successfully' });
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: 'Password successfully updated!' });
      setCurrentPassword('');
      setNewPassword('');
    } catch (err: any) {
      const errorMessage =
        err.data?.message || err.message || 'Failed to update password. Please try again.';
      setSnackbar({ open: true, message: errorMessage, severity: 'error' });
    }
  };
  ...
  useEffect(() => {
    if (brandingData) {
      ...
      setOriginalBranding({
        ...
        foreground: fgColor,
        background: bgColor,
        occasion: contextBranding.occasion || 'default',
      });
      setOccasion(contextBranding.occasion || 'default');
    }
  }, [brandingData]);
              ...
              <AccordionDetails sx={{ px: 3, pt: 2, pb: 3 }}>
                <Stack spacing={3}>
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2, mt: 1 }}>
                    <PrimaryButton onClick={handleSaveEmail} disabled={!hasEmailChanges}>
                      Save email settings
                    </PrimaryButton>
                  </Box>
                </Stack>
            ...
            >
              ...
              <AccordionDetails sx={{ px: 3, pt: 2, pb: 3 }}>
                <Stack spacing={3}>
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2, mt: 1 }}>
                    <PrimaryButton onClick={handleSaveBranding} disabled={!hasBrandingChanges}>

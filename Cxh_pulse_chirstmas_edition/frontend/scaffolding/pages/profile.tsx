...
export default function Page() {
  ...
  const [userProfile, setUserProfile] = useState<AuthUser | null>(user);
              ...
              <Box sx={{ flex: 1,display: 'flex', flexDirection: { xs: 'column',sm: 'column', md: 'column' }, alignItems: {xs: 'center', sm:'flex-start', md: 'flex-start', lg: 'flex-start'}}}>
                ...
                >
                  <Typography variant="caption" sx={{ fontWeight: 600 }}>
                    {userProfile?.roleMappings?.[0]?.role?.name || 'User'}
                  </Typography>
                </Box>
              </Box>
            </Stack>
            ...
            <AccordionDetails sx={{ px: 3, pt: 2, pb: 3 }}>
              <Stack spacing={3}>
                ...
                <TextField
                  ...
                  label="Role"
                  name="role"
                  value={userProfile?.roleMappings?.[0]?.role?.name || 'User'}
                  size="medium"
                  helperText="Role is assigned by administrators"
                />
                <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2, mt: 1 }}>
                  <PrimaryButton onClick={handleSubmit} disabled={disableBtn}>
                    Save changes
                  </PrimaryButton>
                </Box>
              </Stack>

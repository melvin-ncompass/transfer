...
import { createSwark5Architecture } from "./commands/create-swark5-architecture";
...
function registerCommand(context: vscode.ExtensionContext): void {
    ...
    // Swark 5.0 - Advanced Analysis with Filtering & Cleaning
    const swark5Disposable = vscode.commands.registerCommand("swark5.advancedAnalysis", async () => {
        await createSwark5Architecture();
    });

    context.subscriptions.push(disposable, chunkedDisposable, swark3DemoDisposable, swark3Disposable, commitDisposable, swark4Disposable, swark5Disposable);
}

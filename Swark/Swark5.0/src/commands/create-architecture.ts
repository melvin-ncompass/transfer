...
export class CreateArchitectureCommand {
    public static async run(): Promise<void> {
        ...
        try {
            ...
            // Check if files were truncated due to token limits and suggest chunked approach
            const config = vscode.workspace.getConfiguration("swark2");
            const maxFiles = config.get<number>("maxFiles", 1000);
            
            if (files.length >= maxFiles * 0.8) {
                const useChunked = await vscode.window.showInformationMessage(
                    `Large repository detected. Some files may have been skipped due to size limits. For complete analysis of all files, try the chunked approach.`,
                    "Switch to Chunked Analysis",
                    "Continue with Current Analysis"
                );
                
                if (useChunked === "Switch to Chunked Analysis") {
                    vscode.window.showInformationMessage("Switching to chunked analysis for complete coverage...");
                    vscode.commands.executeCommand("swark2.chunkedArchitecture");
                    return;
                }
            }

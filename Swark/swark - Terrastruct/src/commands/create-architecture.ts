...
1. **Render the Diagrams**: Use the links below to open them in D2 Playground (free) or TerraStruct (requires account), or install the [D2 VS Code extension](https://marketplace.visualstudio.com/items?itemName=terrastruct.d2) for local rendering.
...
`;
        ...
        // Extract D2 code and create proper links for each diagram
        const highLevelD2 = this.extractD2Code(highLevelResponse);
        const semiD2 = this.extractD2Code(semiResponse);
        const detailedD2 = this.extractD2Code(detailedResponse);

        const highLevelLinks = this.createD2Links(highLevelD2);
        const semiLinks = this.createD2Links(semiD2);
        const detailedLinks = this.createD2Links(detailedD2);

        const highLevelSection = `## 🏗️ High-Level Architecture Diagram
*Overview diagram for stakeholders and business understanding*

**D2 Playground**: [Edit](${highLevelLinks.view}) | **TerraStruct**: [Open](${highLevelLinks.terrastruct})
...
**D2 Playground**: [Edit](${semiLinks.view}) | **TerraStruct**: [Open](${semiLinks.terrastruct})
...
**D2 Playground**: [Edit](${detailedLinks.view}) | **TerraStruct**: [Open](${detailedLinks.terrastruct})
...
`;
    ...
    private static extractD2Code(response: string): string {
        try {
            const d2Block = OutputFormatter.getD2Block(response);
            return d2Block.replace(/```d2|```/g, "").trim();
        } catch (error) {
            console.error("Error extracting D2 code:", error);
            return ""; // Return empty string if extraction fails
        }
    }

    private static createD2Links(d2Code: string): { view: string; edit: string; terrastruct: string } {
        try {
            if (!d2Code) {
                return {
                    view: "https://play.d2lang.com/",
                    edit: "https://play.d2lang.com/",
                    terrastruct: "https://app.terrastruct.com/"
                };
            }
            ...
            return {
                ...
                terrastruct: linkGenerator.createTerraStructLink()
            };
        } catch (error) {
            console.error("Error creating D2 links:", error);
            return {
                view: "https://play.d2lang.com/",
                edit: "https://play.d2lang.com/",
                terrastruct: "https://app.terrastruct.com/"

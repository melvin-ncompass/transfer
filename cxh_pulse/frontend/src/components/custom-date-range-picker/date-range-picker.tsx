...
const DateRangeSlider: React.FC<DateRangeSliderProps> = ({
    ...
    const applyStartDate = () => {
        ...
        const isValid =
            newFrom < dateRange.to && // Start must be before end (not equal)
            startYear >= minYear &&
            startYear <= maxYear &&
            (startYear > minYear || startMonth >= effectiveMinMonth) &&
            (startYear < maxYear || startMonth <= effectiveMaxMonth) &&
            // Prevent same month and year as end date
            !(startYear === dateRange.to.getFullYear() && startMonth === dateRange.to.getMonth());
    ...
    const applyEndDate = () => {
        ...
        const isValid =
            newTo > dateRange.from && // End must be after start (not equal)
            endYear >= minYear &&
            endYear <= maxYear &&
            (endYear > minYear || endMonth >= effectiveMinMonth) &&
            (endYear < maxYear || endMonth <= effectiveMaxMonth) &&
            // Prevent same month and year as start date
            !(endYear === dateRange.from.getFullYear() && endMonth === dateRange.from.getMonth());
    ...
    // Check if current frame is within the selected range
    const isFrameInRange = currentFrame
        ? getMonthIndexMemo(currentFrame) >= startIndex && getMonthIndexMemo(currentFrame) <= endIndex
        : false;

    // Calculate thumb position within the selected range (blue bar), not the full track
    // Map frame position in frames array to position between startPercent and endPercent
    const currentFramePercent = currentFrame && frames.length > 0 && isFrameInRange
        ? (() => {
            // Find the index of currentFrame in the frames array
            const frameIndex = frames.findIndex(f => 
                f.getFullYear() === currentFrame.getFullYear() && 
                f.getMonth() === currentFrame.getMonth()
            );
            if (frameIndex === -1) return null;
            
            // Calculate frame's relative position in frames array (0 to 1)
            const framePosition = frames.length === 1 
                ? 0.5  // If only one frame, position it in the middle
                : frameIndex / (frames.length - 1);
            
            // Map to position within selected range (startPercent to endPercent)
            return startPercent + (framePosition * (endPercent - startPercent));
        })()
        : null;
    ...
    return (
        <Box
            ...
            tabIndex={0}
        >
            {/* Date buttons and slider - always on same line */}
            <Box sx={dateRangePickerStyles.dateControls}>
                <Button
                    variant="outlined"
                    size="small"
                    onClick={(e) => {
                ...
                <DatePickerPopover
                    ...
                    onClose={handleDateStartPickerClose}
                    title="Start Date"
                    month={startMonth}
                    year={startYear}
                    ...
                    otherDate={{ year: endYear, month: endMonth }}
                ...
                <DatePickerPopover
                    ...
                    onClose={handleDateEndPickerClose}
                    title="End Date"
                    month={endMonth}
                    year={endYear}
                    ...
                    otherDate={{ year: startYear, month: startMonth }}
                    isFullscreen={isFullscreen}
                    anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }}
                />
            </Box>

            {/* Play controls - wraps to new line on mobile, same line on desktop */}
            {playable && (
                <Box sx={dateRangePickerStyles.playControls}>
                    <PlayableControls
                        isPlaying={isPlaying}
                        isLooping={isLooping}
                        showAll={showAll}

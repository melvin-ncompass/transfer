...
export function getLayerStyle({
    ...
    // In subcounty mode, keep unselected subcounties visible but without color fill
    if (selectionMode === 'subcounty' && !isSelected) {
        return {
            color: '#999999',
            weight: 1,
            opacity: 0.6,
    ...
    // Default: very low opacity for unselected features (ward mode with selection)
    return {
        color: '#cccccc',
        weight: 1,
        opacity: 0.3,

...
export function useIndicatorAnimation({
  ...
  const setPlayingUser = (value: boolean) => {
    if (value) {
      ...
      if (delta >= frameInterval) {
        setFrameIdx((i) => {
          ...
          if (isLoopingRef.current) {
            const wrappedIdx = nextIdx % maxFrames;
            return wrappedIdx;
          } else {
  ...
  useEffect(() => {
    if (!playing) return;
    if (!userPlayRef.current) return;

    if (!isLooping && frames.length > 0 && frameIdx >= frames.length - 1) {
      setFrameIdx(0);
    }

    // consume the user action
    userPlayRef.current = false;
  }, [playing, isLooping, frameIdx, frames.length]);

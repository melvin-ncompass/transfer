...
function PermissionNode({
  ...
  const toggleNode = () => {
    ...
    setForm((prev) => {
      ...
      // When unchecking, also check if parent should be unchecked
      let newPermissions = prev.permissions.filter((p) => !allNames.includes(p));
      
      // Walk up and uncheck parents if all their children are unchecked
      let currentParent = parentNode;
      while (currentParent) {
        const siblingNames = currentParent.children?.map((c: any) => c.name) || [];
        const anySiblingChecked = siblingNames.some((name: string) => newPermissions.includes(name));
        if (!anySiblingChecked) {
          // All siblings unchecked, remove parent too
          newPermissions = newPermissions.filter((p) => p !== currentParent.name);

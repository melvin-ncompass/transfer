...
export function CreateRoleForm({
  ...
  const { data: permissions, isLoading: permissionsLoading } = useGetPermissionsQuery(undefined, { skip: !hasRolePermission || !open });
  ...
  const toggleNode = useCallback((node: Permission, parentMapRef: Map<string, Permission>) => {
    ...
    setForm((prev) => {
      ...
      if (allSelected) {
        ...
        // Walk up and uncheck parents if all their children are unchecked
        let currentId = node.id;
        while (parentMapRef.has(currentId)) {
          const parent = parentMapRef.get(currentId)!;
          const allChildrenUnchecked = parent.children?.every(
            (child: Permission) => !newSet.has(child.id)
          ) ?? true;
  ...
  // Build parent map for quick lookup (id -> parent Permission)
  const parentMap = useMemo(() => {
    const map = new Map<string, Permission>();
    const buildMap = (nodes: Permission[], parent?: Permission) => {
      ...
      <Box>
        ...
        >
          ...
          <Checkbox
            ...
            onChange={() => toggleNode(node, parentMap)}
            sx={{
              py: 0.5,
              color: 'text.secondary',
              '&.Mui-checked': { color: 'primary.main' },
              '&.MuiCheckbox-indeterminate': { color: 'primary.main' },
              '&.Mui-disabled': { color: 'text.disabled', opacity: 0.6 },
            }}
          />
        ...
        >
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
            ...
            >
                  ...
                  helperText={
                    isDefault ? (
                      <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 0.5 }}>
                        Default role name cannot be edited
                      </Typography>
                    ) : form.name.length > 49 ? (
              ...
              {/* Permissions Section */}
              <Box>
                {!isShow && <Typography variant="subtitle2" sx={{ mb: 2, fontWeight: 600 }}>
                  Permissions *
                </Typography>}
                <Box
                  sx={{
            ...
            >
              ...
              >
                <Typography variant="body2" color="text.secondary">
                  {form.selectedPermissionIds.size} permission{form.selectedPermissionIds.size !== 1 ? 's' : ''} {isShow === true ? "given" : "selected"}
                </Typography>
                {isShow === true ?
                  ""
                  :
                  <PrimaryButton
                    ...
                    startIcon={isUpdating || isLoading ? <Iconify icon="svg-spinners:180-ring" width={20} /> : undefined}
                  >

...
import { SessionsSwagger } from './swagger/sessions.swagger';
...
export class SessionController {
  ...
  @SwaggerEndpoint(SessionsSwagger, 'SESSION_ACTIVITY_LOGS')
  ...
  @ApiExtraModels(UserLogResponseDto)
  @SwaggerEndpoint(SessionsSwagger, 'USER_ACTIVITY_LOGS')
  @UseGuards(JwtAuthGuard, PermissionsGuard)
  @Permissions(PermissionEnum.READ_SESSION_LOGS)
  async getAllLogs(@Query() query: PaginatedSessionFilterDto) {
    if (query.limit && query.page) {
      const { logs, total } =

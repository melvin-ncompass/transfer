...
import { viewAll } from './user.controller';
...
export class UserService {
  ...
  async getAllUsers( query: viewAll) {
  ...
  async getAllUsersV2( query: viewAll) {
    this.logger.log(`getAllUsersV2 called`);
    const { page, limit, search, roleFilter, status } = query;
    ...
    try {
      ...
      const usersQuery = userRepo
        ...
        .leftJoinAndSelect('user.roleMappings', 'roleMappings')
        .leftJoinAndSelect('roleMappings.role', 'role')
        .orderBy('userInfo.name', 'ASC');

      if (search) {
        usersQuery.andWhere(
          new Brackets((qb) => {
            qb.where('userInfo.name ILIKE :parameter', { parameter: `%${search}%` })
              .orWhere('userInfo.email ILIKE :parameter', { parameter: `%${search}%` })
              .orWhere('userInfo.phone ILIKE :parameter', { parameter: `%${search}%` });
          }),
        );
      }

      if (roleFilter) {
        usersQuery.andWhere('role.id = :roleId', { roleId: roleFilter });
      }
      ...
      function searchUserInfo(search?: string) {
        if (!search) {
          return undefined;
        }
        return [
          { userInfo: { name: ILike(`%${search}%`) } },
          { userInfo: { email: ILike(`%${search}%`) } },
          { userInfo: { phone: ILike(`%${search}%`) } },
        ];
      }
      ...
      const enrichedInvites = await Promise.all(
        filteredInvites.map(async (invite) => {
          ...
          const isExpired = invite.inviteExpiry
      ...
      let finalInvites = enrichedInvites;
      let finalRequests = enrichedRequests;

      if (roleFilter) {
        finalInvites = enrichedInvites.filter((invite) =>
          invite.userInfo.roles.some((r) => r.id === roleFilter),
        );
        finalRequests = enrichedRequests.filter((request) =>
          request.userInfo.roles.some((r) => r.id === roleFilter),
        );
      }
      ...
      if (page && limit) {
        ...
        const userCount = {
          ...
          totalInvites: finalInvites.length,
        ...
        return { flattened_users, total, userCount };
      }
      return flattened;
    } catch (error) {
  ...
  async updateUserRoleByEmail(dto: UpdateUserRoleDto) {
    ...
    try {
      ...
      const deniedRequest = await queryRunner.manager
        .createQueryBuilder(SysUserRequest, 'request')
        .innerJoin('request.userInfo', 'userInfo')
        .where('userInfo.email = :email', { email })
        .andWhere('request.status = :status', { status: 'denied' })
        .getOne();

      if (deniedRequest) {
        this.logger.warn(
          `Cannot update role for rejected user with email: ${email}`,
        );
        throw new BadRequestException(
          `Cannot update role for rejected user ${email}`,
        );
      }
      ...
      if (user) {
        ...
        await queryRunner.manager
          ...
          .update(SysUserInfo)
          .set({ roleIds: [role.id] })
          .where('id = :id', { id: user.userInfo.id })
          .execute();
  ...
  async hardDeleteUser(emails: string[]) {
    ...
    try {
      ...
      const users = await userRepo
        ...
        .leftJoinAndSelect('user.roleMappings', 'roleMappings')
        .leftJoinAndSelect('roleMappings.role', 'role')
        .where('userInfo.email IN (:...emails)', { emails })
        .getMany();

      // Find all invites and requests with matching emails
      const invites = await userInviteRepo
        .createQueryBuilder('invite')
        .leftJoinAndSelect('invite.userInfo', 'userInfo')
        .where('userInfo.email IN (:...emails)', { emails: emails })
        .getMany();

      const requests = await userRequestRepo
        .createQueryBuilder('request')
        .leftJoinAndSelect('request.userInfo', 'userInfo')
        .where('userInfo.email IN (:...emails)', { emails })
        .getMany();

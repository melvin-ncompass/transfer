...
export class ProfileService {
  ...
  async findById(id: string) {
    ...
    try {
      ...
      const { password, roleMappings, ...safeUser } = user;
      const roleName = roleMappings[0]?.role?.name;      
  ...
  async sendResetLink(email: string) {
    ...
    const subject = `Password Reset Instructions for ${BrandingEnum.COMPANY_NAME}`;
    // const title = `Reset Your ${BrandingEnum.COMPANY_NAME} Password`;

    const bodyLines = [
      `Hello${user.userInfo.name ? ` ${user.userInfo.name}` : ''},`,
      `A password reset has been initiated for your ${BrandingEnum.COMPANY_NAME} account. You may use the link below to establish a new password. If you did not initiate this action, you may disregard this email.`
    ];

    const footerLines = [
  `<strong>Please note:</strong> This link is time-sensitive and will expire in 15 minutes. If you did not initiate this action, you may safely disregard this email; your current password will remain unchanged.`
];
